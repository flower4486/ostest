...
u32 read_elf(phyaddr_t cr3, void* load_addr);
...
void kernel_main(void)
{
	...
	for (int i = 0 ; i < PCB_SIZE ; i++, p_proc++) {
		...
		lcr3(p_proc->pcb.cr3);    // map_kernel映射内核程序对应的页表
		...
		read_file(filename[i], (void *)K_PHY2LIN(48 * MB));     // 从磁盘中读文件
		p_proc->pcb.user_regs.eip =  read_elf(p_proc->pcb.cr3, (void*)K_PHY2LIN(48 * MB));
		...
		// 需要你自行处理给用户分配用户栈。
		map_user_stack(p_proc->pcb.cr3);                         // 从3G开始，向低地址生长
...
u32 read_elf(phyaddr_t cr3, void* load_addr)
{
	struct Elf * elf_head_ptr = load_addr;
	//找到程序头表入口地址
	struct Proghdr * prog_head_ptr = (struct Proghdr *)((unsigned char *)load_addr + elf_head_ptr->e_phoff);   
	//找到程序被加载到的虚拟地址，并以此为依据为程序分配存储空间
	int load_num = elf_head_ptr->e_phnum;
	map_user_proc(cr3, prog_head_ptr->p_va);   // 为每个用户进程分配6个页
	load_num -= 1;
	while (load_num--)  //加载前四个程序头
	{
		memcpy((void *)prog_head_ptr->p_va, (void *)((unsigned char *)load_addr + prog_head_ptr->p_offset)
		, prog_head_ptr->p_filesz);
		prog_head_ptr++;
	}
	return elf_head_ptr->e_entry;
}