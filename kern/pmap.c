...
void
map_user_stack(phyaddr_t cr3)                                           // 映射栈的空间
{
	uintptr_t *pde_ptr = (uintptr_t *)K_PHY2LIN(cr3);
	pde_ptr += PDX(3 * GB - 4 * KB);                                // 预计分配4kb用户栈空间，故只分配一个页目录表，一个页表,一个页
...
void
map_user_proc(phyaddr_t cr3, u32 addr_to_map)
{
	// 初始化pde（页目录表）
	// 由于cr3是物理地址，需要进行一步转化转化到线性地址才能访存
	uintptr_t *pde_ptr = (uintptr_t *)K_PHY2LIN(cr3);                     // 页目录表入口指针
	// 基地址由程序头表决定
	pde_ptr += PDX(addr_to_map);                                          // 页目录表内偏移
	// 一个页目录项对应一个页表，一个页表包含1024个页表项，一个页表项对应4K的内存空间
...
map_kern(phyaddr_t cr3)
{
	...
	uintptr_t *pde_ptr = (uintptr_t *)K_PHY2LIN(cr3);
	// 基地址在3GB处
	pde_ptr += PDX(3 * GB);                  // 用于索引一个页目录项，此时的3GB是内核程序的起始地址
	// 一个页目录项映射4MB的内存，计算需要初始化的页目录项数目：一个页目录项对应一个页表，一个页表包含1024个页表项，一个页表项对应4K的内存空间
	int pde_num = (128 * MB) / (4 * MB);     // 32个页目录项 
	// 被映射的物理地址？
	phyaddr_t phy_addr = 0; 
	while (pde_num--) {
		// 对于每一个页目录项申请一页用于页表
